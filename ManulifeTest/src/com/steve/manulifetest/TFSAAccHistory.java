...
import com.steve.manulife.adapter.TFSAHistoryAdapter;
...
public class TFSAAccHistory extends Activity {
	TitleBar mTitleBar;
	ListView mListView;
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		setContentView(R.layout.tfsa_acc_history);
		mTitleBar = (TitleBar)findViewById(R.id.tfsaacchistory_titlebar);
		...
		TFSAAccHistoryTask task = new TFSAAccHistoryTask(this);
		task.execute(); 
	}
	
	class TFSAAccHistoryTask extends AsyncTask<String, String, AccTransactionHistory[]>{
		AccTransactionHistory[] accTransactionHistorys;
		Context context;
		public TFSAAccHistoryTask(Context context) {
			this.context = context;
		}
		@Override
		protected AccTransactionHistory[] doInBackground(String... params) {
			try{
				 InputStream stringData = getAssets().open("TfsaAccount.json");
		...
		protected void onPostExecute(AccTransactionHistory[] result) {
			//UI synchronize data 
			mListView = (ListView) findViewById(R.id.tfsa_listview);
			//integrate into a list of activity
			mListAdapter = new TFSAHistoryAdapter(context, Utils.addIntoArraylist(result));
