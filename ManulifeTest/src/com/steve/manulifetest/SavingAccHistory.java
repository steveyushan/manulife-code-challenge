...
import com.steve.manulife.adapter.SavingHistoryAdapter;
...
public class SavingAccHistory extends Activity {
	TitleBar mTitleBar;
	ListView mListView;
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		setContentView(R.layout.saving_acc_history);
		mTitleBar = (TitleBar)findViewById(R.id.savingacchistory_titlebar);
		...
		SavingAccHistoryTask task = new SavingAccHistoryTask(this);
		task.execute(); 
	}

	class SavingAccHistoryTask extends AsyncTask<String, String, AccTransactionHistory[]>{
		AccTransactionHistory[] accTransactionHistorys;
		Context context;
		public SavingAccHistoryTask(Context context) {
			this.context = context;
		}
		@Override
		protected AccTransactionHistory[] doInBackground(String... params) {
			try{
				 InputStream stringData = getAssets().open("savingsAccount.json");
		...
		protected void onPostExecute(AccTransactionHistory[] result) {
			//UI synchronize data 
			mListView = (ListView) findViewById(R.id.saving_listview);
			//integrate into a list of activity
			mListAdapter = new SavingHistoryAdapter(context, Utils.addIntoArraylist(result));
